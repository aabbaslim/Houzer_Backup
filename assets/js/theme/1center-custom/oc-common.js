...
export default function (context) {
  ...
  $(window).on('load', function () {
    setTimeout(function () {
      let title = $(".form-option-wrapper input:radio:checked").next('.form-option').find('span').attr('data-title');
      if(title !== ''){
        $(".variation-img").hide();
        $(".variation-img[data-label="+title+"]").show();
      }
    },10);
  });

  $(document).on('click', ".form-option-wrapper", function () {
    let title = $(".form-option-wrapper input:radio:checked").next('.form-option').find('span').attr('data-title');
    if(title !== ''){
      $(".variation-img").hide();
      $(".variation-img[data-label="+title+"]").show();
  ...
  $(document).ready(function () {
    ...
    $('.search-cancle').click(function (e) {
      $(this).parents('.dropdown').removeClass('is-open');
      $(this).parents('.desktop-search').find('.quickSearchResults').hide();
      $(this).parents('.desktop-search').find('.quickSearchResults').removeClass('search-activated');
    });
    $(document).on('click', '.view-all', function () {
      $(this).closest('.search-wrapper').find('form').submit();
      $(this).parents('.desktop-search').find('form').submit();
    });
    ...
    // PDP-tabs-accordina
    $(document).on('click', '.oc-description-tabrow .tab', function () {
      $(this).parents('.oc-description-tabrow').find('.tab.active').not(this).each(function () {
        $(this).click();
      });
    ...
    if ($(".oc-description-tabrow").length > 0) {
      $(".oc-description-tabrow:first").find(".tab").trigger("click");
    }
...
function ocbrandlogoslider() {
  ...
  let sliderEl = $('.oc-brand-logos .custom-row');
  if ($(window).width() > 240) {
    sliderEl.not('.oc-brand-logos .custom-row.slick-initialized').slick({
...
function categoryPageSlider() {
  ...
  if ($(window).width() < 1024) {
    if($('.sub-category-block .sub-category-item').length >= 3) {
...
function homebelowbotomproducts() {
  let sliderEl = $('[data-content-region=home_below_bottom_products] [data-sub-layout-container]').not('.oc-home-bottom-products [data-content-region=home_below_bottom_products] [data-sub-layout-container]');
  if (window.innerWidth <= 1180) {
    let styleslector = $("[data-content-region=home_below_bottom_products] [data-sub-layout-container] > [data-container-styling]").not('.oc-home-bottom-products [data-content-region=home_below_bottom_products] [data-sub-layout-container] > [data-container-styling]');

    $(styleslector).detach();
    $('[data-content-region="home_below_bottom_products"] [data-layout-id]').not('.oc-home-bottom-products [data-content-region="home_below_bottom_products"] [data-layout-id]').append(styleslector);
    sliderEl.not('[data-content-region=home_below_bottom_products] [data-sub-layout-container].slick-initialized, .oc-home-bottom-products [data-content-region=home_below_bottom_products] [data-sub-layout-container]').slick({
      infinite: true,
      autoplay: true,
      speed: 300,
      slidesToShow: 1,
...
function relatedproduct() {
  let sliderEl = $('.oc-related-product .productCarousel');
  if (window.innerWidth <= 1023) {

    sliderEl.not('.oc-related-product .productCarousel.slick-initialized').slick({
      dots: false,
      arrows: false,
      infinite: true,
      autoplay: true,
...
function couchForLiving(c) {
  if (window.location.pathname == '/') {
    ...
    let opt = {
      template: "1center-custom/couch-for-living",
      config: {
        products: {}
      }
    }
