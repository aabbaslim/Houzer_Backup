...
export class GetProductOptions {
  ...
  getGraphQlData(ids, start) {
    ...
    fetch('/graphql', {
      ...
      let finalData = {}, productURLs = {}, productVariants = {};
      ...
      res.data.site.products.edges.map(prod => prod.node && prod.node.variants?.edges?.length > 0 && ((productVariants[prod.node.entityId] = prod.node.variants.edges)));
      this.makeHTMLDataForProductOptions(finalData, productURLs, productVariants);
  ...
  makeHTMLDataForProductOptions(products, productURLs, productVariants) {
    const that = this;
    let moreSwatch;
    ...
    Object.entries(products).forEach(([key, value]) => {
        ...
        value.forEach(opt => {
          ...
          if (data.values?.edges?.length > 1 && data.displayStyle.toLowerCase() === 'swatch') {
            ...
            data.values?.edges?.slice(0, 4).forEach(option => {
              ...
              //for BV variants for listing page
              $(`.bv-ratings[data-product-id="${key}"]`).attr('data-bv-product-id', productVariants[key][0].node.sku);
      ...
      //const $variantTargetEl = document.querySelectorAll(`.card .bv-ratings[data-product-id="${key}"]`);
    });
  }
