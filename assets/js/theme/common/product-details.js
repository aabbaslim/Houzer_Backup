...
export default class ProductDetails extends ProductDetailsBase {
    constructor($scope, context, productAttributesData = {}) {
        ...
        $form.on('submit', event => {
            ...
            if (this.addToCartValidator.areAll('valid')) {
               return this.addProductToCart(event, $form[0]);
            }
        });
        ...
        setTimeout(()=>{
            this.detailPageCustomize(this.$scope);
            this.pdpFancyboxScript();
        });
                ...
                if (checked) {
                    ...
                    if (type === 'set-rectangle' || type === 'set-radio' || type === 'product-list') {
                        ...
                        if (label) {
							...
							$(".productView-thumbnail").each(function(a,b) {
								$(this).hide();
								if($(this).find('img').attr('alt') == '' || $(this).find('img').attr('alt').toLowerCase() == 'main' || $(this).find('img').attr('alt') == `${label}`) {
									$(this).show();
									$getVisibleImages.push(a);
								}
							});
							$(".productView-img-container.slider-single").find('a[rel="gallery"]').each(function(a,b) {
								$(this).attr('data-fancyvisible','hide');
								if($.inArray(a, $getVisibleImages) > -1) {
									$(this).attr('data-fancyvisible','show');
								}
							});
                        }
                    }
                    ...
                    if (type === 'swatch') {
                        ...
                        if (label) {
							...
							$(".productView-thumbnail").each(function(a,b) {
								$(this).hide();
								if($(this).find('img').attr('alt') == '' || $(this).find('img').attr('alt').toLowerCase() == 'main' || $(this).find('img').attr('alt') == `${label.title}`) {
									$(this).show();
									$getVisibleImages.push(a);
								}
							});
    ...
    productOptionsChanged(event) {
        ...
        utils.api.productAttributes.optionChange(productId, $form.serialize(), 'products/bulk-discount-rates', (err, response) => {
            ...
            $(".form-input--incrementTotal").attr('max', response.data.stock);
    ...
    listenQuantityChange() {
        this.$scope.on('click', '[data-quantity-change] button', event => {
            ...
            if ($target.data('action') === 'inc') {
                ...
                if($input.val() >= max) {
                    alert(`The max value is ${max}. Please add less than max value.`);
                    $input.val('1');
                    return false;
                }
        ...
        this.$scope.on('keyup', '.form-input--incrementTotal', (event) => {
            let max = parseInt($(event.currentTarget).attr('max'));
    ...
    addProductToCart(event, form) {
        ...
        let qty = parseInt($(form).find("[data-quantity-change] input").val());
        let minqty = parseInt($(form).find("[data-quantity-change] input").data("quantity-min"));
        let maxqty = parseInt($(form).find("[data-quantity-change] input").data("quantity-max"));

        if(minqty > 0 && qty < minqty) { 
            showAlertModal(`Min ${minqty} quantity is required to add this product to cart`);
            return false;
        }

        if(maxqty > 0 && qty > maxqty) { 
            showAlertModal(`Max ${maxqty} quantity is required to add this product to cart`);
            return false;
        }
    ...
    detailPageCustomize(scop) {
        
        if($(".slider-single",scop).length > 0){    
            
            let allImgs = ProductDetails.pdpMainSliderImageUrl(scop);
    ...
    static pdpMainSliderImageUrl(scop=null){        
        let Elmnt;
        if(scop){
    ...
    static RestoreOldImg(scop,getImageS){                          
        if($(".productView-image .slick-slide",scop).length > 0){
            $(".productView-image .slick-slide",scop).each((idx,el)=>{                      
                let currentImg = getImageS[$(el).data("slick-index")];                
                if(currentImg){                
    ...
    pdpFancyboxScript() {                
        ...
        const getAllImgs = () =>{
			...
			if($(".productView-img-container a").attr('data-fancyvisible') !== undefined){
				$(".productView-img-container a[data-fancyvisible='show']").each((i,elmnt)=>{

					mainImageUrl = $(elmnt).attr("href");
				...
				$(".productView-img-container a").each((i,elmnt)=>{
            ...
            $(".productView-thumbnail:visible",this.$scope).each((i,elmnt)=>{
                let opt = {}
                let imgurl = $(elmnt).find('.productView-thumbnail-link').attr('href');
                
                opt['src'] =  imgurl;
                if($(elmnt).hasClass("videoGallery-item")){
                    let videoID = $(elmnt).find('[data-video-id]').length > 0? $(elmnt).find('.productView-thumbnail-link').data("video-id"):0;
                    if(videoID){
        ...
        $(".productView-img-container",this.$scope).on("click",(event) =>{
            ...
            let iddx = sessionStorage.getItem("currentIdx");

            $.fancybox.getInstance().jumpTo(iddx);    
            //console.log(iddx);        
        })

        $(".productView-thumbnail",this.$scope).on("click",(event)=>{                         
            event.preventDefault();                                                 
            sessionStorage.setItem("currentIdx",$(event.currentTarget).index());

            console.log(sessionStorage.getItem("currentIdx"));
        });
